...
known_issues:
  ...
  - "Switching cards: B's article top converges below viewport y=0 by a content-dependent residual (steady state, not an ease residual) since the deferred-collapse unification (5b96fd9) — ~42px on the 2026-05 content, ~86px on the 2026-06 content where the first featured card (asics-running, 2 photos) has equal expanded/collapsed heights so the expand-done handler's delta compensation is a no-op. Cosmetically minor; the anchored-switch test threshold guards intent (near the top, not wildly off) at <120px, not the exact residual. If anchor precision ever matters, the scroll-compensate delta in the photo-card-expand-done handler is the place to look. Audit 2026-06-12."
  - "Zero-delta open/close (expanded height === collapsed height, e.g. a 2-photo featured project) fires no height transitionend; FeaturedPhotoCard finishes those phases via watchdog timers (OPEN/CLOSE_DURATION_MS + 120ms) added 2026-06-12 — without them the card jammed in an animating phase (stuck aria-expanded, dead clicks, sibling never collapsing)."
---
